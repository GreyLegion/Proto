...
include 'constructComment_tree.php';##'check_connection_database.php'; ##подключаем файл для использования функцыи checkConnection_database()
$name = $_POST['n'];
$ucomment = $_POST['ucomment'];

if ($name!=NULL && $ucomment!=NULL){
	$mysqli1 = Connection_database();## вызов функцыи из файла check_connection_database.php
	$ins = "INSERT INTO  `comment_table` (`name` , `comment`) VALUES ('$name', '$ucomment')";
	$w_b = $mysqli1->query($ins);
	$mysqli1->close();
	}
